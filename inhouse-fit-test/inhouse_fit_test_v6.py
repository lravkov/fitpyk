...
def test_pearson7(x, height, center, hwhm, shape):
    # results = amplitude / (((1 + ((x - center)/sigma)**2) * (2**(1 / m) - 1))**m)
    # results = height / (1 + ((x - center) / hwhm) ** 2 * (2 ** (1 / shape) - 1)) ** shape
...
if __name__ == '__main__':
    ...
    mod = Model(test_pearson7) + PolynomialModel(1)
    ...
    hwhm = q.Measurement(true_results[2], true_err[2])
    shape = q.Measurement(true_results[3], true_err[3])

    c0 = q.Measurement(true_results[4], true_err[4])
    c1 = q.Measurement(true_results[5], true_err[5])
    ...
    print(f"hwhm: {hwhm.value} +/- {hwhm.error}")
    print(f"shape: {shape.value} +/- {shape.error}")
    ...
    # print(result.nvarys)
    y_fit = [test_pearson7(x=x, height=height.value, center=center.value, hwhm=hwhm.value, shape=shape.value) + c0.value + (c1.value * x) for x in x]
