...
def process_data_threaded(base_path, project_path, data_dir, storage_dir_name, file_prefix, file_spacer, file_suffix, data_type, z_fill, q, min_x, max_x, peak_centers, peak_heights, peak_types, bckgrd_pts, half_window, divs, use_poly6, min_cutoff, max_nfev):
    ...
    while not q.empty():
        ...
        file_number = q.get()
        print(f'Remaining in queue: {q.qsize()}')
        ...
        # Store the failed file (data that is cutoff at min_x, max_x)
        if max(result.best_fit) < min_cutoff:
            # Indicate which pattern failed
            print(f'Pattern failed ({file_number}) because max value ({max(result.best_fit)}) was below min_cutoff ({min_cutoff}).')

            # Copy the data to failed file directory
            failed_file = storage_dir + 'failed/' + file_prefix + file_spacer + str(file_number).zfill(
                z_fill) + file_suffix + '_failed' + data_type
            with open(failed_file, "w+") as f:
                for i, y_dat in enumerate(y):
...
if __name__ == '__main__':
    ...
    - add a way to store failed files                                   [DONE]
    - make a way to transfer results between fitpyk_2023 and fityk
    - add a way to increase the number of output points for plotting
    - check if you can used delta WSSR
    """
    ...
    storage_dir_name = 'results_instr/'
    file_prefix = 'CeO'
    file_spacer = '_'
    file_suffix = ''
    ...
    peak_centers = [3.093, 3.583, 5.087, 5.949, 6.220, 7.183, 7.809, 8.045, 8.806, 9.330, 10.158, 10.631, 10.783,
                    11.353, 11.770, 11.910, 12.443, 12.828, 12.956, 13.437]
    peak_heights = [17000, 5000, 10500, 8600, 1600, 1700, 3500, 2100, 3000, 2700, 900, 2300, 1000, 1100, 700, 450, 200,
                    1000, 350, 1000]
    ...
    # Don't allow use of more cores than installed in computer
    if num_cores > multiprocessing.cpu_count():
        invalid_num_cores = num_cores
        num_cores = multiprocessing.cpu_count()
        print(f'Maximum number of cores selected ({invalid_num_cores}) > available cores ({multiprocessing.cpu_count()}). Using maximum available cores ({multiprocessing.cpu_count()}).')
    ...
    # Create a worker for each core and start it
    workers = []
    for t in range(num_cores):
        worker = multiprocessing.Process(target=process_data_threaded, args=(base_path, project_path, data_dir, storage_dir_name, file_prefix, file_spacer, file_suffix, data_type, z_fill, q, min_x, max_x, peak_centers, peak_heights, peak_types, bckgrd_pts, half_window, divs, use_poly6, min_cutoff, max_nfev,))
        worker.start()
        workers.append(worker)

    # Join each worker so we can time the whole fitting process, we have to do this after all processes start
    for _ in workers:
        _.join()
