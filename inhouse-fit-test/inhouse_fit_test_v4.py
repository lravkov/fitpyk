...
def splitpearson7_2(x, center2, amplitude2, sigma_left2, m_left2, sigma_right2, m_right2):
    left = heaviside(x-center2, 0.5)*pearson7(x, amplitude2, center2, sigma_left2, m_left2)
    right = heaviside(center2-x, 0.5)*pearson7(x, amplitude2, center2, sigma_right2, m_right2)
    return left+right
...
if __name__ == '__main__':
    ...
    mod = Model(splitpearson7) + Model(splitpearson7_2) + PolynomialModel(1)
    params = mod.make_params(center=12.11, amplitude=300,
                             sigma_left=0.01, m_left=1,
                             sigma_right=0.01, m_right=4,
                             center2=12.43, amplitude2=300,
                             sigma_left2=0.01, m_left2=1,
                             sigma_right2=0.01, m_right2=4,
                             c0=100, c1=0)
    ...
    center2 = q.Measurement(true_results[6], true_err[6])
    amplitude2 = q.Measurement(true_results[7], true_err[7])
    sigma_left2 = q.Measurement(true_results[8], true_err[8])
    m_left2 = q.Measurement(true_results[9], true_err[9])
    sigma_right2 = q.Measurement(true_results[10], true_err[10])
    m_right2 = q.Measurement(true_results[11], true_err[11])

    c0 = q.Measurement(true_results[12], true_err[12])
    c1 = q.Measurement(true_results[13], true_err[13])
    ...
    print(f"center2: {center2.value} +/- {center2.error}")
    print(f"amplitude2: {amplitude2.value} +/- {amplitude2.error}")
    print(f"sigma left2: {sigma_left2.value} +/- {sigma_left2.error}")
    print(f"m left2: {m_left2.value} +/- {m_left2.error}")
    print(f"sigma right2: {sigma_right2.value} +/- {sigma_right2.error}")
    print(f"m right2: {m_right2.value} +/- {m_right2.error}")
    # FWHM = ((sigma left + sigma right) * np.sqrt(2 * np.log(2))) / 2
    fityk_left_hwhm2 = sigma_left2 * q.sqrt(2 * q.log(2)) / 2
    print(f"fityk left hwhm2: {fityk_left_hwhm2.value} +/- {fityk_left_hwhm2.error}")
    fityk_right_hwhm2 = sigma_right2 * q.sqrt(2 * q.log(2)) / 2
    print(f"fityk right hwhm2: {fityk_right_hwhm2.value} +/- {fityk_right_hwhm2.error}")
    fityk_fwhm2 = fityk_left_hwhm2 + fityk_right_hwhm2
    print(f"fityk fwhm2: {fityk_fwhm2.value} +/- {fityk_fwhm2.error}")
    height2 = splitpearson7_2(x=center2.value, center2=center2.value, amplitude2=amplitude2.value, sigma_left2=sigma_left2.value, sigma_right2=sigma_right2.value, m_left2=m_left2.value, m_right2=m_right2.value)
    print(f"fityk height2: {height2}")
    print(' ')
    ...
    # print(result.nvarys)
    y_fit = [splitpearson7(x=x, center=center.value, amplitude=amplitude.value, sigma_left=sigma_left.value, sigma_right=sigma_right.value, m_left=m_left.value, m_right=m_right.value) + splitpearson7_2(x=x, center2=center2.value, amplitude2=amplitude2.value, sigma_left2=sigma_left2.value, sigma_right2=sigma_right2.value, m_left2=m_left2.value, m_right2=m_right2.value) + c0.value + (c1.value * x) for x in x]
