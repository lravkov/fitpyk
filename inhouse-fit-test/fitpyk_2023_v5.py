...
if __name__ == '__main__':
    """
    TODO:
    - make it store the original as well as bg_subtracted files
    ...
    for file_number in range(first_file, first_file + number_of_spectra):
        ...
        file_path = base_path + data_dir + file_prefix + file_spacer + str(file_number).zfill(z_fill) + file_suffix + data_type
        print(file_path)
        ...
        knot_xvals, knot_yvals = background_generator_V4(bckgrd_pts, half_window, divs, x, y)
        ...
        storage_dir = base_path + storage_dir_name
        no_bg_path = storage_dir + 'no_bg/'
        if not os.path.exists(no_bg_path):
            os.makedirs(no_bg_path)

        no_bg_file = no_bg_path + file_prefix + file_spacer + str(file_number).zfill(z_fill) + file_suffix + '_no_bg' + data_type
        ...
        spline_file = spline_path + file_prefix + file_spacer + str(file_number).zfill(z_fill) + file_suffix + '_spline' + data_type
