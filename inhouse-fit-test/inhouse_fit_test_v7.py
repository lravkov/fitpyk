...
def test_pearson7(x, height, center, hwhm, shape):
    # results = amplitude / (((1 + ((x - center)/sigma)**2) * (2**(1 / m) - 1))**m)  # OLD ONE THAT's BAD
    # results = height / (1 + ((x - center) / hwhm) ** 2 * (2 ** (1 / shape) - 1)) ** shape  # NEW ONE THAT'S GOOD
    return height / (1 + ((x - center) / hwhm) ** 2 * (2 ** (1 / shape) - 1)) ** shape


def test_splitpearson7(x, height, center, left_hwhm, left_shape, right_hwhm, right_shape):
    left = heaviside(x - center, 0.5) * test_pearson7(x, height, center, left_hwhm, left_shape)
    right = heaviside(center - x, 0.5) * test_pearson7(x, height, center, right_hwhm, right_shape)
    return left + right
...
if __name__ == '__main__':
    ...
    # print(result.nvarys)
    y_fit = [test_splitpearson7(x=x, height=height.value, center=center.value, left_hwhm=left_hwhm.value, left_shape=left_shape.value, right_hwhm=right_hwhm.value, right_shape=right_shape.value) + c0.value + (c1.value * x) for x in x]
