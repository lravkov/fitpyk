...
if __name__ == "__main__":
    # ------------------------------------------- MAIN INPUTS --------------------------------------------------------
    inputfile = 'Cu_test_data_neutron_long_mwh_output.txt'

    startpattern = 47938  # 43 for hypotenuse up triangles
    numrows = 1
    numcols = 31
    prefix = 'Cu_test'
    suffix = 'longitudinal'

    plottingthis = 'csds'  # csds, rho, q

    filteredrows = []  # [17, 24]
    # the method to filter cols is in the code below

    writeflag = False
    filelocation = 'P:/PyCharmProjectsP/botting/venv/fitpyk-2024-jun/test-data/longitudinal_results/'  # put / at the end
    ...
    print(len(z))
    newz = []
    for rpos, row in enumerate(z):
        newrow = []

        # USE THIS STRUCTURE TO FILTER HORIZONTAL LINES
        if rpos not in filteredrows:  # 17, 24
            for vpos, val in enumerate(row):
                newrow.append(np.nanmean(val))

                # USE THIS STRUCTURE TO FILTER VERTICAL LINES
                # if vpos != 16:
                #     newrow.append(np.nanmean(val))
                # else:
    ...
    if plottingthis == 'csds':
        ...
        z_min, z_max = 1e15, 2e16  # rhoas
    else:
        z_min, z_max = 1.32, 2.13  # qas
