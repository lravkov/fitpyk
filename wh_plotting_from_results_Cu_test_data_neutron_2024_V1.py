...
if __name__ == "__main__":
    ...
    base_path = 'P:/PyCharmProjectsP/botting/venv/fitpyk-2024-jun/test-data/transverse_results/'  # include / at end of base_path
    data_filename = 'combined_results_all_params.txt'
    rel_error = 0.02  # 0.02, 0.029, 0.05 are good values

    # -180--170
    info = "'trans'"  # include the inner quotes
    ...
    # input your lower and upper window bounds for where peaks are located
    peakwindow_lower = 14.0
    peakwindow_upper = 14.6
    max_fwhm = 0.11  # 0.026, 0.060, 0.090 are good values
    print_to_file_flag = False
    mwh_worthy_filename = 'mWH-worthy-Cu-test-data-neutron-trans-14-146.txt'
    ...
    c = ax.pcolormesh(x, y, z, cmap='nipy_spectral', vmin=z_min, vmax=z_max)
    ax.set_title(f'Test - FWHM [Q] vs. Location - +/- {rel_error * 100}%')
