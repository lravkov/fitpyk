...
{
double k,dk,k_edge, p,fw, shape, fwhm, stepsize;
double k_filenev;
char *filenev;
FILE *profile;
long int i, i_max;
...
if (argv[1] == NULL || argv[2] == NULL || argv[3] == NULL || argv[4] == NULL || argv[5] == NULL) {printf("\n\nUsage: ./program_name peak_position fwhm shape number_of_datapoints MAX_value_of_deltaK\n\n"); exit(-42);}
else;
...
p=1/pow((1+pow(((k-0)/(fwhm*0.5)),2)*(pow(2,(1/shape))-1)),shape);
