...
def unpack_all_results(base_path_p, data_filename_p):
    ...
    for i in range(len(data_temp_p)):
        ...
        if (data_name_p[0].split()[0][0] == 'D'):
            ...
            temp_magic_number = int(magic_string.split('.')[0][::-1].split('_')[0][::-1]) - 20000  # number matters: 5000, 10000, 20000
...
if __name__ == "__main__":
    base_path = 'D:/PyCharmProjects/fityk_scripting_2022/LANL-CuFe-foils/SinSqPsi-LPA/200_200/'
    data_filename = 'K_results_2_combined.txt'
    ...
    # REMEMBER TO CHANGE THE NUMBER IN THE unpack_all_results METHOD
    bad_list = [2, 4]  # [5, 9] for 50_50, [10, 12] for 100_100, [2, 4] for 200_200
    ...
    for bank in range(1, 16):
        ...
        if bank not in bad_list:
            for i, pt in enumerate(xpos_to_plot):
                # print(pt)
                if 13.4 < pt < 13.7:  # 6.8 to 7.2 for BCC 1, 7.6 to 8.0 for FCC 1, 12.8 to 13.2 for BCC high, 13.4 to 13.7 for FCC high
                    print(pt)
        ...
        if bank not in bad_list:
            print(f'PATTERN: {bank}')
            print(f'k_pos: {xpos_to_plot}')
            print(f'delta_k: {ypos_to_plot}')
            print(f'pattern: {bank}, average FWHM: {np.nanmean(ypos_to_plot)}')
    ...
    plt.xlabel('Centre [K]')
    plt.ylabel('FWHM [delta K]')
    plt.title('FWHM vs. Centre, LPA200, instrumentals not subtracted')
    plt.xlim([minx, maxx])
    plt.ylim([0, 0.09])
    plt.show()
    # interactive(True)
    ...
    plt.title('LPA50 - Cu (422) - FWHM vs. Pattern')  # low BCC = 200 (6.8:7.2), low FCC = 220 (7.6:8.0), high BCC = 321 (12.8:13.2), high FCC = 422 (13.4:13.7)
    plt.xlabel('Pattern')
    plt.ylabel('FWHM [delta K]')
