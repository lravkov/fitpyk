...
def unpack_instrumental_sh(file):
    data_temp_p = pd.read_table(file)
    # print(data_temp_p)

    centres = []
    fwhms = []
    for pos, row in enumerate(data_temp_p.iloc[:, 0]):
        if pos > 0:
            # print(row)
            splitrow = row.split()
            # print(splitrow)
            centres.append(float(splitrow[1]))
            fwhms.append(float(splitrow[2]) - float(splitrow[3]))
    return centres, fwhms

if __name__ == "__main__":
    # get the instrumental data from the instrumental file we already generated
    data_filename = 'LANL_CuFe_foils_instrumental.sh'
    centres, fwhms = unpack_instrumental_sh(data_filename)
    print(centres)
    print(fwhms)

    # write instrumentals to file
    # with open(r'D:/PyCharmProjects/fityk_scripting_2022/LANL-CuFe-foils/LANL-CuFe-instrumentals.txt', 'w') as f:
    #     # write a header
    #     f.write('# centre [K], fwhm [delta K]\n')
    #     for pos, item in enumerate(centres):
    #         # write each item on a new line
    #         f.write(f'{item}, {fwhms[pos]}\n')

    base_path = 'D:/PyCharmProjects/fityk_scripting_2022/LANL-CuFe-foils/LPA_50/50-51/'
    ...
    # plt.plot(xpos, FWHM, marker='o', linestyle='', markersize=3.5, markerfacecolor='none', label=f'CuFe-LPA-50')
    ...
    plt.title('FWHM vs. Centre - instrumental')
    plt.xlim([minx, maxx])
    # plt.ylim([0, 0.09])  # use this if plotting both
    plt.ylim([-0.005, 0.085])  # use this if just plotting instrumentals
    plt.show()
    interactive(True)
