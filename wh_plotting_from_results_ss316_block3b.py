...
def unpack_all_results(base_path_p, data_filename_p):
    ...
    for i in range(len(data_temp_p)):
        ...
        # check to see if the line is not the polynomial6, and also that the line is not a header
        if (data_name_p[0].split()[1] != 'Polynomial6') and (len(param_list_p) > 1):
            print('--------' + str(i) + '--------')
            print(data_name_p)
            print(param_list_p)
...
if __name__ == "__main__":
    base_path = 'D:/PyCharmProjects/fityk_scripting_2022/ANSTO_DATA/GE-1-ss316block3b/110--100_110100_mar_18/155-166/'
    ...
    # Lhwhmpoly = np.polyfit(CELHa[idxL], LHediteda[idxL], 2)  # output is: [ 5.42776074e-05  8.77460852e-04 -1.39327986e-03] ax^2 + bx + c
    # print(Lhwhmpoly)
    #
