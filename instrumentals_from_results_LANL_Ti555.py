...
def unpack_all_results(base_path_p, data_filename_p):
    ...
    for i in range(len(data_temp_p)):
        ...
        # THIS GIVES YOU THE PATTERN NUMBER -> MAY NEED TO CHANGE IT
...
if __name__ == "__main__":
    base_path = 'D:/PyCharmProjects/fityk_scripting_2022/LANL-Ti-5553_instrumentals/35-36/'
    ...
    plt.figure()
    plt.plot(xpos, FWHM, marker='o', linestyle='', markersize=3.5, markerfacecolor='none', label=f'Instrumentals')
    plt.plot(xpLHWHM, pLHWHM(xpLHWHM), '-', label='2nd order fit')
    plt.legend(loc='upper left')
    plt.xlabel('Center [2theta]')
    plt.ylabel('FWHM [delta 2theta]')
    plt.title('FWHM vs. Center, instrumental LaB6')
    plt.xlim([minx, maxx])
    plt.ylim([0.01, 0.02])
    plt.show()
    # interactive(True)
